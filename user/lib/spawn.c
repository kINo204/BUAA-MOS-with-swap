...
int spawn(char *prog, char **argv) {
	...
	if (init_stack(child, argv, &sp) < 0) { goto err1; }
	...
	ELF_FOREACH_PHDR_OFF (ph_off, ehdr) {
		...
		if (ph->p_type == PT_LOAD) {
			...
			if (read_map(fd, ph->p_offset, &bin) < 0) { goto err1; }
			...
			if (elf_load_seg(ph, bin, spawn_mapper, &child) < 0) { goto err1; }
		}
	}
	close(fd);
